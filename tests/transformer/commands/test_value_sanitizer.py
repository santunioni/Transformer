...
from transformer.commands.value_sanitizer import ValueSanitizer, ValueSanitizerConfig
...
class TestValueSanitizer(unittest.TestCase):
    def test_aggregate_keys(self):
        ...
        self.transformer_config = ValueSanitizerConfig(
            name="value-sanitizer",
            key_pattern="^(email_).*",
    ...
    def test_cpf_mask_remover(self):
        ...
        self.transformer_config = ValueSanitizerConfig(
            name="value-sanitizer", key_pattern="cpf", substitution_pattern="[^0-9]"
        )
        self.transformer = ValueSanitizer(config=self.transformer_config)
        transformed_data = self.transformer.transform(data, {})
        self.assertEqual(target_data, transformed_data)
